...
class CryptoQuantProvider:
    ...
    def __init__(self, api_key: Optional[str] = None):
        ...
        if not self.api_key:
            raise ValueError("CRYPTOQUANT_API_KEY required in .env file")
    ...
    def _fetch_metric(self, 
        ...
        extra_params: pass as tuple of (key, value) pairs to avoid lru_cache issues with dicts.
        ...
        # Base params — note: miner-flows endpoints use 'miner', not 'exchange'
        if endpoint.startswith('miner-flows'):
            params = {
                'window': window,
                'miner': 'all_miner',
                'limit': min(limit, 7)
            ...
            params = {
                'window': window,
                'exchange': 'all_exchange',  # Required for exchange-flows endpoints
                'limit': min(limit, 7)
            }
        
        # Merge any extra caller-specified params
        ...
        try:
            ...
            if status_code == 404:
                print(f"⚠️  CryptoQuant endpoint not found (404): {endpoint}")
                print(f"     Endpoint may have changed in current API tier — skipping gracefully.")
    ...
    def get_exchange_reserve(self, days: int = 7) -> pd.DataFrame:
        """
        ...
        """
        return self._fetch_metric('exchange-flows/reserve', limit=min(days, 7))

    def get_miner_outflow(self, days: int = 7) -> pd.DataFrame:
        """
        Large-holder proxy — delegates to get_exchange_netflow().
        (Fix 5: miner-flows/* unavailable on current CryptoQuant tier)
        Negative netflow = BTC leaving exchanges = accumulation (bullish).
        """
        return self.get_exchange_netflow(days)

    def get_miner_to_exchange(self) -> pd.DataFrame:
        """Backward-compat alias. Delegates to get_miner_outflow."""
        return self.get_miner_outflow()
